...
class LatticeGas{
  private:
    ...
    double p,p0;
public:
  LatticeGas(double p1,double p2);
  void Borrar();
...
LatticeGas::LatticeGas(double p1,double p2){
  V[0]=V[3]=1; V[2]=V[1]=-1;
  p0=p1; p=p2;
}
...
int main(int argc,char** argv){
  double p0 = (double) std::atof(argv[1]);
  double p = (double) std::atof(argv[2]);
  int N = std::atoi(argv[3]);
  int tmax = std::atoi(argv[4]);
  LatticeGas Difusion(p0,p);
  Crandom ran64(1);
  double mu=Lx/2, sigma=16;
  int t;
  std::ofstream file("var.dat");
  ...
  for(t=0;t <tmax;t++){
    ...
    Difusion.Adveccione();
    file << t << "\t" << Difusion.Varianza() <<std::endl;
  }
  file.close();
  return 0;
}
