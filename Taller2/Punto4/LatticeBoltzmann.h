...
public:
  ...
  void Collision(void);
  void ImposeFields(double Ufan, double omega, int ixc, int iyc, int R);
  void Advection(void);
  ...
  void print_deriv(const char * NameFile);
};
...
void LatticeBoltzmann::ImposeFields(double Ufan, double omega, int ixc, int iyc, int R){
  int i,ix,iy,n0; double rho0, Ux0, Uy0, R2=R*R;
  ...
  for(ix=0;ix<Lx;ix++)
	...
	for(i=0;i<Q;i++){
	  ...
	  n0=n(ix,iy,i); fnew[n0]=feq(rho0,Ux0,Uy0,i);
	}
    }
}
...
void LatticeBoltzmann::print_deriv(const char * NameFile){
