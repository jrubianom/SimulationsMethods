...
const double n1=1,n2=2;
const double alpha = 1; // el alfa que suavisa tan ( x - x0 )/alpha

const int Lx=400;
const int Ly=200;

const double theta_i = 60;
const double theta = (90-theta_i)*M_PI/180;
...
double LatticeBoltzmann::Ccelda(int ix,int iy){
  double v,nCelda, iy0 = Ly/2.0,x0=100;
  double ix0 = (iy-iy0)/tan(theta) + x0;
  nCelda = (n2-n1)*0.5*tanh((ix-ix0)/alpha) + (n2+n1)*0.5;
  v = C/nCelda;
  return v;
}
