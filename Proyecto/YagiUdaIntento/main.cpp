...
int main(){
  ...
  //Driven elements
  double lxs=0,lys=0,lzs=lamd*0.45,betaS=5;
  vector<Sources> JSources;
  JSources.push_back(Sources(ixs,iys,izs,lxs,lys,lzs,lamd,C,J0,betaS,alpha,D3d));

  //Parasitic elements
  double lxd=2,lyd=2,lzd=lamd*0.56; double lxr=2,lyr=2,lzr=lamd*0.55;
  double SigmaP = 1,beta = 0.5;
  double ixd = ixs+(int)(lamd*0.125); double ixr = ixs - (int)(lamd*0.35);
  vector<Parasitic> Paras;
  Paras.push_back(Parasitic(ixd,iys,izs,lxd,lyd,lzd,SigmaP,beta)); //Director
  Paras.push_back(Parasitic(ixr,iys,izs,lxr,lyr,lzr,SigmaP,beta)); //Reflector
