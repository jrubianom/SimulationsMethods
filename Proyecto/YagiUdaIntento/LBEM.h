...
class LatticeBoltzmann{
  ...
  public:
    LatticeBoltzmann(Parameter Params0,vector<Parasitic> Paras0,
                     vector<Sources> JSources0);
    ~LatticeBoltzmann();
    ...
    void Print(string fileB,string fileE,string fileS);
};


LatticeBoltzmann::LatticeBoltzmann(Parameter Params0,vector<Parasitic> Paras0,
                                   vector<Sources> JSources0){

  Paras = Paras0; JSources=JSources0;
  ...
  t=0;
  ixs = Params0.ixs; iys = Params0.iys; izs = Params0.izs;
...
double LatticeBoltzmann::sigma(int ix,int iy,int iz){
  double s=0;

  for(auto& ParasElement : Paras)
    s += ParasElement.Sigma(ix,iy,iz);
...
vector3D LatticeBoltzmann::JprimaSource(int ix,int iy,int iz){
  vector3D Jprima0; Jprima0.cargue(0,0,0);
  for(auto&  SourceElement : JSources)
    Jprima0 += SourceElement.JSource(ix,iy,iz,t);
  return Jprima0;
}
  ...
  for(ix=0;ix<Lx;ix++) //para cada celda
    for(iy=0;iy<Ly;iy++)
      for(iz=0;iz<Lz;iz++){
        ...
        Jprima0=JprimaSource(ix,iy,iz)+Jprima(E0,prefactor0);
        Eprima0=Eprima(E0,Jprima0,epsilonr0);
        //BGK evolution rule
        id0 = index0(ix,iy,iz);
...
void LatticeBoltzmann::Print(string fileB,string fileE,string fileS){
  int ix,iy=iys,iz,r,p,i,j;
  ...
  for(ix=0;ix < Lx; ix++){
    //for(iz=0;iz<Lz;iz++){
      ...
      if(iz==Lz/2){
        ...
        file<<ix<<" "<<abs(E2*B2)*0.5*pow(lambda,2)<<" "<<abs(Eteo*Bteo)*0.5*pow(lambda,2)<<endl;
...
void LatticeBoltzmann::PowerOverSphere(double R,int Ntheta,int Nphi,vector<double> &V){
  ...
  for(int i=0; i < Nphi; i++){
    ...
    for(int j=0; j < Ntheta; j++){
      ...
      x += ixs; y += iys; z += izs; //centered in the dipole coordinates
      V[i*Ntheta+j] = PowerAtPoint(x,y,z);
    }
  }
}
...
double LatticeBoltzmann::PowerAtPointTheta(double x,double y){
  ...
  double s1,s2,s3,s4,s5,s6,s7,s8;
  s1 = Poynting(x1,y1,izs);
  s2 = Poynting(x1+1,y1,izs);
  s3 = Poynting(x1,y1+1,izs);
  s4 = Poynting(x1+1,y1+1,izs);
...
double LatticeBoltzmann::PowerAtPointPhi(double x,double z){
  ...
  double s1,s2,s3,s4,s5,s6,s7,s8;
  s1 = Poynting(x1,iys,z1);
  s2 = Poynting(x1+1,iys,z1);
  s3 = Poynting(x1,iys,z1+1);
  s4 = Poynting(x1+1,iys,z1+1);
  return Interpolatebi(s1,s2,s3,s4,x,z);
}


void LatticeBoltzmann::PowerPlane(double R,int Ntheta,double phi0,vector<double> &V){
  double theta, dtheta = M_PI/(Ntheta/2-1);
  double x,y,z;
...
double LatticeBoltzmann::Poynting(int ix,int iy,int iz){
  ...
  vector3D S = E^H;
  vector3D r; r.cargue(ix-ixs,iy-iys,iz-izs);
  //cout << ix << " " <<iy << " " << iz <<
  //norma(E)/(Z0*J0) << " " << norma(B)/J0 << " " << norma(E)*norma(B)/(Z0*J0*J0)*pow(norma(r),2)*0.5 << endl;
  //return norma(S);
  return (S*r)*norma(r);
  //return norma(E)*norma(B)*pow(norma(r),2);
}
