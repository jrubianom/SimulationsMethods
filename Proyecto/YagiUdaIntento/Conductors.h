...
class Parasitic{
    ...
    public:
        Parasitic(int x0,int y0,int z0,double lx0,double ly0,double lz0,double SigmaP0,double beta0=2);
        friend class LatticeBoltzmann;
        friend class Theo;
        double Sigma(int ix,int iy,int iz);
};

Parasitic::Parasitic(int x0,int y0,int z0,double lx0,double ly0,double lz0,double SigmaP0,double beta0){
    x = x0; y=y0; z=z0;
...
double Parasitic::Sigma(int ix,int iy,int iz){
    ...
    double sx = (1-tanh(beta*(ax-ix)))*(1-tanh(beta*(ix-bx)));
    double sy = (1-tanh(beta*(ay-iy)))*(1-tanh(beta*(iy-by)));
    double sz = (1-tanh(beta*(az-iz)))*(1-tanh(beta*(iz-bz)));
    return SigmaP*sx*sy*sz/4.0;
}
