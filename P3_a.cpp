...
/*
El siguiente programa fija un dt, y realiza la evolución del
sistema Sol-Júpiter con el método PEFRL. El tiempo de la
simulación es tal que se realizan 20 órbitas.
Además, se produce un archivo de datos "P3_a.dat" con los
datos de posición en cada paso.
Si se redirige el output a gnuplot, se produce una gráfica del
movimiento completo.
*/
...
void Interaccionador::calcule_fuerzas_entre(Cuerpo &planeta1, Cuerpo &planeta2){
  ...
  F = G*m1*m2*r21/std::pow(d,3);
  planeta1.sumar_fuerza(F);
}

void gnuplot_output(){
  std::cout << "set terminal pdf" << std::endl;
  std::cout << "set output 'P3_a.pdf'" << std::endl;
  std::cout << "set size ratio -1" << std::endl;
  std::cout << "plot \"P3_a.dat\" u 1:2 w l, \"\" u 3:4 w l" << std::endl;
}
