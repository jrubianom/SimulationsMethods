...
/*
Se utiliza la misma plantilla del inciso B, pero
modelando tres cuerpos. El troyano se inicializa
formando un triángulo equilatero con el Sol y
Júpiter, cuyo lado es igual a la distancia Sol-Júpiter.
La velocidad inicial se calcula como el producto cruz
de la velocidad angular y la posición.
Al correr el programa y redirigir el output a gnuplot
se genera una gráfica de la posición en el tiempo de los
tres cuerpos.
*/
...
int main() {
  ...
  double m2=0.005;
  double x2=r*std::cos(M_PI/3)+x0, y2=r*std::sin(M_PI/3);
  ...
  for(t=0, tdibujo=0 ; t<tmax; t+=dt, tdibujo+=dt){
    ...
    if(tdibujo>tcuadro){
      fout << posicion_rotada[0].x() << " " << posicion_rotada[0].y() << " "
	   ...
	   << posicion_rotada[2].x() << " " << posicion_rotada[2].y() << "\n";
      tdibujo=0;
    }
...
void gnuplot_output(){
  ...
  std::cout << "set output 'P3_c.pdf'" << std::endl;
  std::cout << "set size ratio -1" << std::endl;
  std::cout << "plot \"P3_c.dat\" u 1:2 w l t 'Sol', \"\" u 3:4 w l t 'Jupiter', \"\" u 5:6 w l t 'Trojano'" << std::endl;
}
